...
export const authOptions: AuthOptions = {
      ...
      async authorize(credentials, req) {
        try {
          const res = await fetch(
            `
            https://barber-service-backend.vercel.app/api/v1/auth/signin`,
            {
              method: "POST",
  ...
  callbacks: {
    ...
    async session({ session, token }: { session: any; token: any }) {
      //   console.log(session, "session authOption");
      ...
      if (!verifiedToken) {
        // console.log("token expired new token generate");
